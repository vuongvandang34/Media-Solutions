...
export const metadata = {
  title: 'Business Feedback CRM | Multi-tenant SaaS Feedbacks Deck',
  description: 'Streamline customer surveys, manage staff permissions, and review sentiment metrics in one isolated CRM tenant workspace.',
};
    ...
    <div className="min-h-screen bg-slate-950 font-sans text-slate-200 antialiased relative overflow-hidden flex flex-col justify-between">
      ...
      <main className="relative z-10 mx-auto max-w-4xl px-6 py-20 text-center space-y-8 flex-1 flex flex-col justify-center">
        ...
        <div className="mx-auto inline-flex items-center gap-1.5 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-3.5 py-1 text-xs font-semibold text-indigo-400">
          ...
          <span>SaaS Phase 1 Infrastructure Active</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto pt-8 border-t border-slate-800/40 text-left text-xs sm:grid-cols-2">
          {highlights.map((text, idx) => (
            <div key={idx} className="flex items-center gap-2 text-slate-450">
      ...
      {/* Landing Footer */}
      <footer className="relative z-10 border-t border-slate-900 bg-slate-950/80 px-6 py-8 text-center text-xs text-slate-500">
        <div className="mx-auto max-w-6xl flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
          <span>© 2026 FeedbackCRM SaaS Inc. All rights reserved.</span>
          <div className="flex justify-center gap-6">
            <span className="flex items-center gap-1">
              <ShieldCheck className="h-4 w-4 text-slate-650" />
              Secure 256-bit JWT Session Layer
            </span>
          </div>
        </div>
      </footer>
